...
export default function ConfirmBook({ setBooks, toConfirm, setToConfirm }) {
    ...
    const handleSubmit = async (e) => {
        ...
        axios.post("book/add-confirmed", { ...toConfirm, payed })
            .then(res => {
                setToConfirm(null)
                setBooks(prev => prev.filter(book => book._id !== res.data.book._id))
    ...
    return (
        <div className="full-cont">
            <form onSubmit={handleSubmit} className="confirm-book">
                ...
                <div className="date-wrapper">
                    <div className="dates">
                        <div className="date">
                            <h2>From:</h2>
                            <input type="date"
                                value={new Date(toConfirm.from).toISOString().split('T')[0]}
                                onChange={e => setToConfirm(prev => ({ ...prev, from: new Date(e.target.value).setHours(roomStart, 0, 0, 0) }))}
                            />
                        </div>
                        <div className="date">
                            <h2>To:</h2>
                            <input type="date"
                                value={new Date(toConfirm.to).toISOString().split('T')[0]}
                                onChange={e => setToConfirm(prev => ({ ...prev, to: new Date(e.target.value).setHours(roomStart, 0, 0, 0) }))}
                            />
                        </div>
                    </div>
                    <h2>({formatDistance(toConfirm.from, toConfirm.to)})</h2>
                </div>
                <hr />
                ...
                <hr />
                <div className="bttns">
                    <button type="submit" className="green">Confirm</button>
